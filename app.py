...
# 2. 데이터 불러오기 (korea.csv 반영)
@st.cache_data
def load_data():
    df = pd.read_csv(r"C:\Users\user\OneDrive\수행평가(2026)\korea.csv")
    return df

try:
    df = load_data()
except Exception as e:
    st.error(f"⚠️ 데이터를 불러오는데 실패했습니다. 경로와 파일명을 다시 확인해주세요.")
    st.info(f"현재 설정된 경로: C:\\Users\\user\\OneDrive\\수행평가(2026)\\korea.csv")
    st.stop()
...
) * 1000000  # 슬라이더 입력을 실제 명 수 단위로 변환

# 4. 수리 모형 방정식 계산
pred_malthus = N0 * np.exp(r_malthus * t_range)
pred_logistic = K_logistic / (1 + ((K_logistic - N0) / N0) * np.exp(-r_logistic * t_range))

# 5. 수학적 오차 계산 (Mean Absolute Error)
...
with col1:
    st.metric(label="📊 맬서스 모형 평균 오차 (MAE)", value=f"{mae_malthus:,.0f} 명")
with col2:
    st.metric(label="📈 로지스틱 모형 평균 오차 (MAE)", value=f"{mae_logistic:,.0f} 명")

st.caption("💡 팁: 오른쪽 범례(Legend)에서 항목을 마우스로 클릭하면 해당 그래프 선을 켜거나 감출 수 있습니다.")
...
fig.add_trace(go.Scatter(
    x=years, y=actual_pop, mode='lines+markers', name='실제 대한민국 인구',
...
# ② 맬서스 예측선
fig.add_trace(go.Scatter(
    x=years, y=pred_malthus, mode='lines', name='맬서스 모형 예측',
...
# ③ 로지스틱 예측선
fig.add_trace(go.Scatter(
    x=years, y=pred_logistic, mode='lines', name='로지스틱 모형 예측',
